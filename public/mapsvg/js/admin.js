...
(function( $ ) {
    ...
    methods = {
        ...
        loadMapFromRemoteSettings: function(instance,url){
            this.afterLoad = instance.methods.getData().options.afterLoad;
            var _this = this;
            instance.methods.setAfterLoad(function(){
                _this.loadMapFromRemoteSettings2(this);
            });
            setTimeout(function(){
                if(!instance.methods.getData().$map.find(".mapsvg-loading").is(":visible")){
                    _this.loadMapFromRemoteSettings2(instance.methods);
                }
            },500);
        },
        loadMapFromRemoteSettings2: function(mapsvg){
            if(this.remoteMapLoadStarted){
                return;
            }
            this.remoteMapLoadStarted = true;
            var data = mapsvg.getOptions(false,true);
            data.afterLoad = this.afterLoad;
            _this.loadDashboard(data,mapsvg.getData().$map.attr('id'));
        },
